...
from hostess.utilities import curry, logstamp, timeout_factory, signal_factory
...
class TCPTalk:
    ...
    def __init__(
        ...
        Args:
            ...
            eomstr: end-of-message string to check for
        ...
        try:
            ...
            self.signals = {k: None for k in self.threads.keys()}
    ...
    def tend(self):
        ...
        for k, v in threads:
            ...
            self.threads.pop(k)
            if k == "select":
                self.threads["select"] = self.launch_selector_thread()
            else:
                self.threads[k] = self.launch_io(k)
    ...
    def launch_selector(self) -> dict:
        ...
        try:
            ...
            return {
                "thread": "select",
                "signal": self.signals.get("select"),
                "exception": exception
            }
    ...
    def launch_io(self, name):
        ...
        while self.signals.get(name) is None:
            ...
            try:
                ...
                continue
            peer, peerage = _check_peerage(key, self.peers)
            callback, peersock = key.data, key.fileobj  # explanatory variables
            if (peerage is True) and (callback.__name__ != self.ack.__name__):
        ...
        return {"thread": name, "signal": self.signals.get(name)}
    ...
    def _read(self, conn: socket.socket) -> tuple[bytes, str, str]:
        ...
        try:
            ...
            while reading:
                data, status, reading = _tryread(conn, self.chunksize)
